...
elements.form.addEventListener("submit",async event=>{event.preventDefault();if(!state.workerReady)return;try{const radius=Number(elements.radius.value);if(!Number.isFinite(radius)||radius<1||radius>2000)throw new Error("搜索半径必须在 1 到 2000 光年之间");const center=await getCenter();if(!center)return;state.center=center;state.view.x=center.mapX;state.view.y=center.mapY;const requestId=++state.requestId;state.activeSearch=requestId;setSearching(true);state.results=[];state.selected=null;renderResults();drawMap();state.worker.postMessage({type:"search",requestId,query:{centerLy:{x:center.xLy,y:center.yLy},radius,targetMode:state.targetMode,targetType:elements.targetType.value,excludeFixed:elements.excludeFixed.checked,excludeNamed:elements.excludeNamed.checked}});}catch(error){showError(error.message);}});
...
function renderDetails(result){
  const matched=result.matchedObjects||[result],matchTypes=[...new Set(matched.map(item=>typeName(item.type)))];
  const match=matched.length===1?(result.kind==="planet"?`当前匹配 A${result.objectIndex} · ${typeName(result.type)} · 所属恒星 ${result.starType}`:`当前匹配 恒星 ${result.objectIndex} · ${result.type}${result.subtype?` · ${result.subtype}`:""}`):`当前匹配 ${matched.length} 个${result.kind==="planet"?"行星":"恒星"} · ${matchTypes.join("、")}`;
