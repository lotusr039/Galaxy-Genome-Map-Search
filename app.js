...
function planetLabel(planet){const group=Number.isInteger(planet?.group)?String.fromCharCode(65+planet.group):"A";return`${group}${planet?.groupIndex??planet?.objectIndex??""}`;}
...
function renderDetails(result){
  ...
  const match=result.kind==="mineral"?`当前匹配 ${matched.length} 处矿石 · ${matchTypes.join("、")} · 最高含量 ${number(result.abundance,0)}%`:matched.length===1?(result.kind==="planet"?`当前匹配 ${planetLabel(result)} · ${typeName(result.type)} · 所属恒星 ${result.starType}`:`当前匹配 恒星 ${result.objectIndex} · ${result.type}${result.subtype?` · ${result.subtype}`:""}`):`当前匹配 ${matched.length} 个${result.kind==="planet"?"行星":"恒星"} · ${matchTypes.join("、")}`;
  ...
  const rows=planets.map(planet=>{
    const label=`${planetLabel(planet)}${planet.name?` · ${planet.name}`:""}`;
